...
#   endif
	...
	// ----------------------------------------------- CUSTOM GRADIENT FOG STUFF


	float4 ApplyGradientFog(float4 c, float _FogStart, float _FogEnd, sampler2D _GradientFog) {
	
		float distance = length(ObjSpaceViewDir(0));
		float depthValue = saturate((distance - _FogStart)/(_FogEnd - _FogStart));
		float4 fogColor = tex2Dlod(_GradientFog, float4(depthValue, 0, 0, 0)); // Color we pick from the Gradient Texture using the depthValue
		return lerp(c, float4(fogColor.rgb, c.a), c.a * fogColor.a);
	}
