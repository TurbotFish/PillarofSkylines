Shader "Alo/Blur/GaussianBlur5x5"
{
	Properties
	{
	...
	CGINCLUDE
		...
		half4 SampleWithDepthCheck(float2 uv, float4 offset12, float4 offset34) {
		...
		Pass {//0
			...
			CGPROGRAM
				...
				half4 HorizontalFragmentProgram (Interpolators i) : SV_Target	{
					...
					float4 uv12 = float4(-2,0,-1,0);
					float4 uv34 = float4(1,0,2,0);

					return SampleWithDepthCheck(i.uv, uv12, uv34);
				}
			ENDCG
		}
		...
		Pass {//1
			...
			CGPROGRAM
				...
				half4 VerticalFragmentProgram (Interpolators i) : SV_Target	{
					...
					float4 uv12 = float4(0,-2,0,-1);
					float4 uv34 = float4(0,1,0,2);
